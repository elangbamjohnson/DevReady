...
test.describe('Curriculum Study Guide Fallback & Navigation', () => {
  test('renders curriculum study guide cleanly without 404 for unwritten topics', async ({ page }) => {
    ...
    const response = await page.goto('/learn/concurrency/checked-and-unsafe-continuations');
    expect(response?.status()).toBe(200);

    // Verify study guide elements
    await expect(page.getByText(/Curriculum Study Guide/i)).toBeVisible();
    await expect(page.getByRole('heading', { level: 1, name: /Bridging Legacy APIs with CheckedContinuation/i })).toBeVisible();
    ...
    await expect(practiceLink).toHaveAttribute('href', /interview\/configure\?topicId=concurrency-continuations/);
  });
});
