...
test.describe('Curriculum Study Guide Fallback & Navigation', () => {
  test('renders curriculum study guide cleanly without 404 for unwritten topics', async ({ page }) => {
    ...
    const response = await page.goto('/learn/swift/initialization-lifecycle');
    expect(response?.status()).toBe(200);

    // Verify study guide elements
    await expect(page.getByText(/Curriculum Study Guide/i)).toBeVisible();
    await expect(page.getByRole('heading', { level: 1, name: /Initialization & Deinitialization Rules/i })).toBeVisible();
    ...
    await expect(practiceLink).toHaveAttribute('href', /interview\/configure\?topicId=swift-initialization/);
  });
});
